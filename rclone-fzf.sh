...
_sysz_help() {
  cat >&2 <<EOF
A utility for using rclone interactively with fzf.
...
OPTS:
  ...
  -a, --authorize          Only show --user units
  -V, --verbose            Print the rclone command
...
CMD:
  l, ls                 rclone ls <unit>
  lsd, lsdir            rclone lsd <unit>
  lsf                   rclone lsf <unit>
  lsjson                rclone lsjson <unit>
  lsl                   rclone lsl <unit>
  t, tree               rclone tree <unit>
  cp, copy              rclone copy <unit>
  copyto                rclone copyto <unit>
  s, sync               rclone sync <unit>
  m, mkdir              rclone mkdir <unit>
  mv, move              rclone move <unit>
  moveto                rclone moveto <unit>
  delete                rclone delete <unit>
  deletefile            rclone deletefile <unit>
  rmdir                 rclone rmdir <unit>
  rmdirs                rclone rmdirs <unit>
  size                  rclone size <unit>
  touch                 rclone touch <unit>
  about                 rclone about <unit>
  authorize             rclone authorize <unit>
  config                rclone config <unit>
  genautocomplete       rclone genautocomplete <unit>
  h, help               rclone help <unit>
  c, cat                rclone cat <unit>
  ed, edit              rclone edit <unit>
  reload                rclone reload <unit>

  If no command is given, one or more can be chosen interactively.

ARGS are passed to the rclone command for each selected unit.
...
_fzf_cat() {
  ...
  SYSTEMD_COLORS=1 rclone "$MANAGER" cat -- "$UNIT"
}
...
_fzf_preview() {
  ...
  if [[ $UNIT = *@.* ]]; then
    ...
    SYSTEMD_COLORS=1 rclone "$MANAGER" status --no-pager -- "$UNIT"
  fi
  exit 0
}
...
_sysz_show() {
  ...
  _sysz_rclone "$manager" show "$unit" -p "$2" --value
}
...
_sysz_list() {
  ...
  (
    rclone list-units "${args[@]}"
    rclone list-unit-files "${args[@]}"
  ) | sort -u -t ' ' -k1,1 |
...
for STATE in "${STATES[@]}"; do
  STATE="${STATE##*=}"
  if [[ -n $STATE ]] && ! rclone --state=help | grep -q "^${STATE}$"; then
...
_sysz_daemon_reload() {
  ...
  for RELOAD in "${RELOADS[@]}"; do
    case $RELOAD in
    '[user] daemon-reload')
      _sysz_rclone --user daemon-reload >&2
      ;;
    '[system] daemon-reload')
      _sysz_rclone --system daemon-reload >&2
      ;;
    esac
  done
}
...
for PICK in "${UNITS[@]}"; do
  ...
  for CMD in "${CMDS[@]}"; do
    ...
    status)
      ...
      SYSTEMD_COLORS=1 rclone "$MANAGER" $CMD --no-pager "${ARGS[@]}" -- "$UNIT"
      ;;
    cat | show)
      _sysz_rclone "$MANAGER" "$CMD" "${ARGS[@]}" -- "$UNIT" || exit $?
      ;;
    *)
      # shellcheck disable=2086
      _sysz_rclone "$MANAGER" $CMD "${ARGS[@]}" -- "$UNIT" || CODE=$?
      SYSTEMD_COLORS=1 rclone "$MANAGER" status --no-pager -- "$UNIT"
      if [[ ${#UNITS[@]} -eq 1 ]]; then
        exit $CODE
      fi
